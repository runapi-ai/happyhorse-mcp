...
describe("happyhorse stdio MCP server", () => {
  ...
  it("exposes the model-line tools over the real stdio transport", async () => {
    ...
    client = new Client({ name: "happyhorse-mcp-test", version: "0.1.0" });
    ...
    expect(names).toEqual(["check_pricing","edit_video","get_task","image_to_video","text_to_video"]);
    ...
    for (const model of ["happyhorse-edit-video","happyhorse-image-to-video","happyhorse-character","happyhorse-text-to-video"]) {
