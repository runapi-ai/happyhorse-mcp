<h1 align="center">HappyHorse MCP Server</h1>

<p align="center">
  <strong>Create HappyHorse AI tasks through RunAPI — 3 endpoints, 4 models — then poll status and check pricing. One MCP server, one API key.</strong>
</p>
...
<p align="center">
  <a href="https://www.npmjs.com/package/@runapi.ai/happyhorse-mcp"><img src="https://img.shields.io/npm/v/%40runapi.ai/happyhorse-mcp?style=flat-square&color=blue" alt="npm version"></a>
...
`@runapi.ai/happyhorse-mcp` is a focused MCP server for the **HappyHorse** model line on RunAPI.
It lets an MCP-compatible assistant create HappyHorse tasks, poll their status, and check current pricing — all through a single RunAPI API key.
...
| Tool | Auth | Purpose |
|---|---|---|
| `edit_video` | Yes | Create a HappyHorse task (edit video) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `image_to_video` | Yes | Create a HappyHorse task (image to video) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `text_to_video` | Yes | Create a HappyHorse task (text to video) and optionally poll until it reaches a terminal status. Returns the task id, status, output URLs, and a price snapshot. |
| `get_task` | Yes | Fetch the current status and latest payload for an existing task. |
| `check_pricing` | No | Look up the current price for a HappyHorse model and endpoint. |

---

## Models

HappyHorse covers 4 models across 3 endpoints. Each tool accepts the models listed for it:
...
The assistant calls `check_pricing` to confirm the cost, then `edit_video`, and returns the task id, status, and output URLs.
